...
```{r, echo=FALSE, evale=TRUE, message=FALSE, warning=FALSE}
devtools::load_all(".")
```
...
cpp %>%
  mutate(whz = 
           case_when(agedays < 730 ~ 
                       who_value2zscore(x = htcm, y = wtkg, x_var = "lencm", y_var = "wtkg", sex = sex),
                     agedays >= 730 ~  
                       who_value2zscore(x = htcm, y = wtkg, x_var = "htcm", y_var = "wtkg", sex = sex)))
...
cpp_lt2$whz <- who_value2zscore(x = cpp_lt2$htcm, y = cpp_lt2$wtkg, x_var = "lencm", y_var = "wtkg", 
                 sex = cpp_lt2$sex)

cpp_gt2$whz <- who_value2zscore(x = cpp_gt2$htcm, y = cpp_gt2$wtkg, x_var = "htcm", y_var = "wtkg", 
                 sex = cpp_gt2$sex)
